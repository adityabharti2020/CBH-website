...
const AddAmmenityInput = () => {
  ...
  const [updateData, setupdateData] = useState({newBasePrice:null , newGST : null});
  ...
  const handleUpdateAmmenityData = async () => {
    ...
    try {
      const response = await axios.put(`/api/v1/admin/update/ammenity/data/${activeData._id}`,{
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      dispatch(openSnackbar("ammenity is deleted successfully", "success"));
      dispatch(isLoading(false));
      getAmmenityData();
    } catch (error) {
      console.log(error);
        ...
        <Grid container rowGap={1} columnGap={1}>
          {inputFields?.map((inputField, index) => (
            ...
            >
              ...
              <Box sx={{ position: "absolute", top: -18, right: -17 }}>
                <IconButton
                ...
                <>
                  ...
                  >
                    ...
                    <Box sx={{ position: "absolute", top: -6, right: -5 }}>
                      <IconButton sx={{ color: "#9f2936", fontSize: "18px" }}>
                        <Iconify
